# 规范: 代码审查规则 (Code Review Rules)

## 目的

定义 CheckIt 系统中代码审查规则的核心数据结构和接口，以实现统一的规则定义和执行标准。
...
### 需求:定义审查范式接口 (Paradigm)

系统必须定义 `Paradigm` 接口，用于描述一组审查规则的集合及其配置。该接口必须包含以下字段：
...
- **当** 定义一个 `Paradigm` 对象时
- **那么** 它必须包含 `name`, `description`, `autofix`, `rules` 属性
