// packages/backend/src/ai-adapter/claude.ts — Anthropic Claude adapter
//
// 调用 Anthropic Messages API (https://api.anthropic.com/v1/messages)。
//
// Auth:
// - apiKey: 显式传入(通常从 env ANTHROPIC_API_KEY 读)
// - 或 env: process.env.ANTHROPIC_API_KEY
// - 必须 header: x-api-key + anthropic-version
//
// Model 默认 "claude-sonnet-4-5"。可覆盖(cfg.model)。
...
2. Up to 5 specific rule recommendations from this catalog:
${RULE_CATALOG}
3. Optionally a bundled preset recommendation from:
${PRESET_CATALOG}
...
/** Same robust JSON extraction as the OpenAI adapter. */
function parseReply(raw: string): ChatReply {
  ...
  const suggestions: RuleSuggestion[] = Array.isArray(obj.suggestions)
    ? (obj.suggestions as unknown[]).map((s): RuleSuggestion | null => {
        if (typeof s === 'string') return { id: s, title: s };
        if (typeof s === 'object' && s !== null) {
          const ss = s as { id?: unknown; title?: unknown; tldr?: unknown };
          if (typeof ss.id === 'string') return { id: ss.id, title: typeof ss.title === 'string' ? ss.title : ss.id, tldr: typeof ss.tldr === 'string' ? ss.tldr : undefined };
        }
        return null;
  ...
  const recommendedSets: PresetRecommendation[] = Array.isArray(obj.recommendedSets)
    ? (obj.recommendedSets as unknown[]).map((p): PresetRecommendation | null => {
        if (typeof p === 'string') return { id: p, name: p, description: '' };
        if (typeof p === 'object' && p !== null) {
          const pp = p as { id?: unknown; name?: unknown; description?: unknown };
          if (typeof pp.id === 'string') return { id: pp.id, name: typeof pp.name === 'string' ? pp.name : pp.id, description: typeof pp.description === 'string' ? pp.description : '' };
        }
        return null;
...
export function makeClaudeAdapter(cfg: AdapterConfig = {}): AiAdapter {
  const apiKey = cfg.apiKey ?? process.env.ANTHROPIC_API_KEY;
  const baseUrl = (cfg.baseUrl ?? process.env.ANTHROPIC_BASE_URL ?? DEFAULT_BASE_URL).replace(/\/$/, '');
  const model = cfg.model ?? process.env.ANTHROPIC_MODEL ?? DEFAULT_MODEL;
  ...
  return {
    ...
    async chat(message: string, ctx: ChatContext): Promise<ChatReply> {
      if (!apiKey) {
        throw new Error(
          `claude adapter: missing API key. Set one of:\n` +
          `  - lintany config set ai.api_key sk-ant-...\n` +
          `  - env var ANTHROPIC_API_KEY=sk-ant-...\n` +
          `Or switch: lintany config set ai.adapter local-keyword`,
        );
      }

      const messages: Array<{ role: 'user' | 'assistant'; content: string }> = [];
      if (ctx.history) {
        for (const h of ctx.history) messages.push({ role: h.role, content: h.text });
      }
      ...
      try {
        ...
        if (!resp.ok) {
          ...
          throw new Error(`claude adapter: HTTP ${resp.status} ${resp.statusText}\n${body.slice(0, 200)}`);
        }
        const data = (await resp.json()) as { content?: Array<{ type?: string; text?: string }> };
        const block = data.content?.find((b) => b.type === 'text');
        const content = block?.text;
        if (typeof content !== 'string') {
          throw new Error(`claude adapter: unexpected response shape (no text block)`);
        }
        return parseReply(content);
      } catch (e) {
        if ((e as Error).name === 'AbortError') {
          throw new Error(`claude adapter: request timed out after ${timeoutMs}ms`);
        }
        throw e;
      } finally {
        clearTimeout(timer);
